...
type SeoStatus = "ok" | "attention" | "error";
...
function statusStyle(status: SeoStatus): string {
  if (status === "ok") {
...
function statusIcon(status: SeoStatus): string {
  if (status === "ok") {
    return "✅";
  }
...
export default function BasicSeoToolsPage() {
  const [url, setUrl] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<BasicSeoResult | null>(null);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/tools/basic-seo-tools", {
        method: "POST",
        headers: {
  ...
  return (
    <main className="space-y-6">
      <section className="space-y-2">
        <h1 className="text-3xl font-bold">Basic SEO Tools</h1>
        <p className="text-sm text-gray-600">
          Vérifie les points SEO essentiels d&apos;une page (HTML + headers HTTP, 10 secondes max).
        </p>
      </section>
      ...
      {result ? (
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            <h2 className="text-lg font-semibold">Vérifications SEO</h2>
