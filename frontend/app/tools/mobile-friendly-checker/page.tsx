...
type MobileCheckStatus = "ok" | "attention" | "error";
...
function statusStyle(status: MobileCheckStatus): string {
  if (status === "ok") {
...
function statusIcon(status: MobileCheckStatus): string {
  if (status === "ok") {
    return "✅";
  }
...
export default function MobileFriendlyCheckerPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/tools/mobile-friendly-checker", {
        method: "POST",
        headers: {
  ...
  return (
    <main className="space-y-6">
      <section className="space-y-2">
        <h1 className="text-3xl font-bold">Mobile-Friendly Checker</h1>
        <p className="text-sm text-gray-600">
          Vérifie les signaux de compatibilité mobile d&apos;une page (sans navigateur headless).
        </p>
      </section>
      ...
      {result ? (
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            <h2 className="text-lg font-semibold">Vérifications mobile</h2>
