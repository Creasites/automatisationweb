...
type SslCheckStatus = "ok" | "attention" | "error";
...
function statusStyle(status: SslCheckStatus): string {
  if (status === "ok") {
...
function statusIcon(status: SslCheckStatus): string {
  if (status === "ok") {
    return "✅";
  }
...
export default function HttpsSslCheckerPage() {
  const [url, setUrl] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<HttpsSslResult | null>(null);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/tools/https-ssl-checker", {
        method: "POST",
        headers: {
  ...
  return (
    <main className="space-y-6">
      <section className="space-y-2">
        <h1 className="text-3xl font-bold">HTTPS / SSL Checker</h1>
        <p className="text-sm text-gray-600">
          Vérifie rapidement HTTPS, redirection HTTP et informations SSL (10 secondes max).
        </p>
      </section>
      ...
      {result ? (
        <section className="space-y-4">
          <div className="rounded-lg border border-gray-200 p-4">
            ...
            <p className="mt-2 text-sm">URL testée: {result.normalizedUrl}</p>
            <p className="text-sm">Score simple: {result.score}/100</p>
          </div>
